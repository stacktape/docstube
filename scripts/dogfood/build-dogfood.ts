...
const writeDogfoodProject = async (input: {
  ...
  await Promise.all([
    writeFile(join(srcDir, 'ACCEPTANCE.md'), input.acceptance, 'utf8'),
    ...
    writeFile(
      ...
      [
        ...
        '    title: Acceptance',
        '    path: acceptance.mdx',
        '    brief: Summarize acceptance evidence from PLAN.md and ACCEPTANCE.md.',
        ''
      ].join('\n'),
      'utf8'
    ),
...
export const buildDogfoodDocs = async (input: DogfoodBuildInput = {}): Promise<DogfoodBuildResult> => {
  ...
  const [acceptance, plan, packageRaw] = await Promise.all([
    readFile(join(workspaceDir, 'ACCEPTANCE.md'), 'utf8'),
  ...
  const acceptanceEvidenceCount = acceptance.split(/\r?\n/g).filter((line) => line.startsWith('- `')).length;
  ...
  try {
    ...
    const readme = [
      ...
      `- Acceptance evidence entries: ${acceptanceEvidenceCount}`,
