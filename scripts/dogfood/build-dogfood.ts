...
const productSummary = (product: string): string => {
  const lines = product
    .split(/\r?\n/g)
    .map((line) => line.trim())
...
const writeDogfoodProject = async (input: {
  ...
  await Promise.all([
    writeFile(join(srcDir, 'PRODUCT.md'), input.product, 'utf8'),
    ...
    writeFile(
      ...
      [
        ...
        '  description: Deterministic docs generated from the docstube product spec.',
    ...
    writeFile(
      ...
      [
        ...
        '  - id: overview',
        '    title: Overview',
        '    brief: Explain what docstube is using the product spec and package facts.',
        '  - id: product',
        '    title: Product',
        '    path: product.mdx',
        '    brief: Summarize architecture and hard boundaries from PRODUCT.md.',
        ''
      ].join('\n'),
      'utf8'
    ),
...
export const buildDogfoodDocs = async (input: DogfoodBuildInput = {}): Promise<DogfoodBuildResult> => {
  ...
  const [product, packageRaw] = await Promise.all([
    readFile(join(workspaceDir, 'PRODUCT.md'), 'utf8'),
    readFile(join(workspaceDir, 'package.json'), 'utf8')
  ]);
  const packageJson = JSON.parse(packageRaw) as PackageJson;
  const productSectionCount = product.split(/\r?\n/g).filter((line) => line.startsWith('## ')).length;
  ...
  try {
    ...
    const readme = [
      ...
      `- Product sections: ${productSectionCount}`,
