...
track_install() {
  ...
  payload="$(printf '{"installId":"%s","status":"%s","version":"%s","platform":"%s","installer":"%s","source":"github-release","durationMs":%s,"errorKind":"%s"}' "$INSTALL_ID" "$status" "$safe_version" "$PLATFORM" "macos.sh" "$duration_ms" "$error_kind")"
