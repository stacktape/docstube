...
const postbuildMjs = () =>
  [
    ...
    "import { navItems, siteDescription, siteName, siteUrl } from '../src/theme-build/site-data.mjs';",
    '',
    ...
    "  html.replaceAll(/<script[\\s\\S]*?<\\/script>/giu, ' ')",
    ...
    '      const titleMatch = /<title>(?<title>[^<]+)<\\/title>/iu.exec(html);',
    '      const navItem = byUrl.get(url);',
    '      return {',
    '        title: navItem?.label ?? titleMatch?.groups?.title ?? url,',
    '        url,',
    ...
    "  if (siteDescription) lines.push(`> ${siteDescription}`, '');",
    '  for (const page of pages.toSorted((left, right) => left.url.localeCompare(right.url))) {',
    "    lines.push(`## ${page.title}`, '', `URL: ${normalizeLlmsUrl(page.url)}`, '', page.content ?? '', '');",
    '  }',
    "  return `${lines.join('\\n').trim()}\\n`;",
    '};',
    '',
    'const createSitemapXml = (pages) => {',
    "  const baseUrl = (siteUrl || 'https://docs.example.test').replace(/\\/$/u, '');",
    '  return [',
    '    \'<?xml version="1.0" encoding="UTF-8"?>\',',
    '    \'<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\',',
    '    ...pages.toSorted((left, right) => left.url.localeCompare(right.url)).flatMap((page) => [',
    "      '  <url>',",
    ...
    "    if (errors.length > 0) throw new Error(`Pagefind failed: ${errors.join('; ')}`);",
    '  } finally {',
...
const themeComponentsTsx = () =>
  [
    ...
    '  return <section className="dt-code-group" data-component="CodeGroup">{title ? <h3>{title}</h3> : null}{children}</section>;',
    '}',
    '',
    ...
    '  return <ul className="dt-file-tree" data-component="FileTree">{entries.map((entry) => <li data-kind={entry.kind ?? "file"} key={`${entry.kind ?? "file"}:${entry.path}`}>{entry.path}</li>)}</ul>;',
    '}',
    '',
    'export function PreviousNext({ previous, next }: { previous?: { title: string; href?: string; description?: string }; next?: { title: string; href?: string; description?: string } }) {',
    '  return <nav className="dt-previous-next" data-component="PreviousNext">{previous ? <a href={previous.href}>{previous.title}</a> : <span />}{next ? <a href={next.href}>{next.title}</a> : <span />}</nav>;',
    '}',
    '',
    'export function ComparisonTable({ columns, rows }: { columns: string[]; rows: (string | number | boolean)[][] }) {',
    '  return <table className="dt-comparison-table" data-component="ComparisonTable"><thead><tr>{columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={index}>{row.map((cell, cellIndex) => <td key={cellIndex}>{String(cell)}</td>)}</tr>)}</tbody></table>;',
    '}',
    '',
    'export function ParamTable({ params }: { params: { name: string; type?: string; required?: boolean; description: string }[] }) {',
    '  return <table className="dt-param-table" data-component="ParamTable"><tbody>{params.map((param) => <tr key={param.name}><th>{param.name}{param.required ? " *" : ""}</th><td>{param.type ? <code>{param.type}</code> : null}</td><td>{param.description}</td></tr>)}</tbody></table>;',
    '}',
    '',
    'export function DecisionTree({ decisions, title }: { decisions: { title: string; href?: string; description?: string }[]; title?: string }) {',
    '  return <section className="dt-decision-tree" data-component="DecisionTree">{title ? <h3>{title}</h3> : null}<ul>{decisions.map((decision) => <li key={decision.title}>{decision.href ? <a href={decision.href}>{decision.title}</a> : <strong>{decision.title}</strong>}{decision.description ? <p>{decision.description}</p> : null}</li>)}</ul></section>;',
    '}',
    '',
    'export function Badge({ children, label, tone = "info" }: { children?: ReactNode; label?: string; tone?: Tone }) {',
    '  return <span className={`dt-badge dt-badge--${tone}`} data-component="Badge">{label ?? children}</span>;',
    '}',
    '',
    'export function ApiReference({ children, kind, signature, sourcePath, symbol }: { children?: ReactNode; kind?: string; signature?: string; sourcePath?: string; symbol: string }) {',
    '  return <section className="dt-api-reference" data-component="ApiReference"><h3><code>{symbol}</code>{kind ? <span> {kind}</span> : null}</h3>{signature ? <pre><code>{signature}</code></pre> : null}{sourcePath ? <p>{sourcePath}</p> : null}{children}</section>;',
    '}',
    '',
    'export function Term({ children, id, label }: { children?: ReactNode; id: string; label?: string }) {',
    '  return <a className="dt-term" data-component="Term" href={`/glossary/#${id}`}>{label ?? children ?? id}</a>;',
    '}',
    '',
...
export const createGeneratedSiteAssets = (input: GeneratedSiteAssetsInput): GeneratedSiteAsset[] => [
  ...
  { path: 'scripts/postbuild.mjs', content: postbuildMjs() },
