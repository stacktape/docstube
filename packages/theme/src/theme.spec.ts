...
describe('docstube theme contract', () => {
  ...
  it('builds the self-contained generated Astro docs-site fixture', async () => {
    ...
    try {
      ...
      await execFileAsync(execPath, [join(fixtureRoot, 'scripts', 'postbuild.mjs')], {
        cwd: fixtureRoot,
        env: { ...env, CI: '1', NO_COLOR: '1' },
        maxBuffer: 10 * 1024 * 1024
      });
      ...
      const pagefindScript = await readFile(join(fixtureRoot, 'dist', 'pagefind', 'pagefind.js'), 'utf8');

      expect(pagefindScript).toContain('pagefind');
      ...
      expect(llmsText).toContain('# docstube fixture');
      expect(llmsText).toContain('/guides/install/');
