...
const runCliGenerateYes = async (repoRoot: string): Promise<readonly string[]> => {
  ...
  const result = await runGenerateCommand(
    {
      ...
      start: async () => ({
        ...
        url: 'http://127.0.0.1:0/wizard?session=fixture'
      })
    },
    output.output
  );
  ...
  expect(output.lines).toContain('info:Zero-question mode enabled.');
...
const runCliUpdateAndResolveDirtyPages = async (repoRoot: string, fixture: ProductSmokeFixture): Promise<void> => {
  ...
  const output = captureOutput();
  const update = await runUpdateCommand({ workspaceDir: repoRoot }, output.output);
...
describe('deterministic product smoke', () => {
  it.each(smokeFixtures)(
    'runs the $name fixture through CLI generate, site build, and update',
    async (fixture) => {
      const repoRoot = await makeFixtureRepo(fixture);
      try {
        await runCliGenerateYes(repoRoot);
        ...
        await runCliUpdateAndResolveDirtyPages(repoRoot, fixture);
      } finally {
